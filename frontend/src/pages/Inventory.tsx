...
interface Item {
  ...
  available_quantity: number // Note: Not in schema provided earlier but usually exists. Assuming sold_quantity for now.
...
export default function Inventory() {
  ...
  const { data: items, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // We will fetch from 'items' and calculate days_without_sale client-side for simplicity if view is unreliable
      ...
      return data.map((item: any) => {
        // Calculate days without sale logic just in case View isn't perfect
        let days = 0;
  ...
  const getRowClass = (item: Item) => {
    ...
    // Logic: > 60 days -> Red, > 30 days -> Yellow
  ...
  if (error) return (
    <div className="p-8 text-center text-red-500">
      ...
      <p className="text-xs text-gray-400 mt-4">Verifique se a tabela 'items' existe e se o RLS permite leitura.</p>
    </div>
  )
